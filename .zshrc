...
# WARNING: Do not edit the managed config in ~/.config/zsh-conf/$RC directly.
...
[[ "${ENABLE_MINIMAL_MODE:l}" == "yes" ]] && \
    RC="$ZSH_CONFIG_ROOT/zshrc.mini" || \
    RC="$ZSH_CONFIG_ROOT/zshrc"
...
if [[ "${ENABLE_FANCY_STARTUP:l}" == "yes" ]]; then
    ...
    # available_visual_commands array defined in user.conf file
...
if [[ "${ZSH_BENCHMARK:l}" == "yes" ]]; then
    ...
    if [[ -r "$ZSH_CONFIG_ROOT/lib/_ui.benchmark" ]]; then
        source "$ZSH_CONFIG_ROOT/lib/_ui.benchmark"
    else
